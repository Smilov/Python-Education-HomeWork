...
def virus(id_fridge, key = 'anton'):
    i = 0
    j = 0
    flag = False
    while i < len(id_fridge):
        if id_fridge[i] == key[j]:
            j += 1
            if j == len(key):
                flag = True
                break
...
numbers_fridge = int(input('Количество холодильников, которые необходимо проверить: '))
id_fridge = [input('Номер холодильника: ') for _ in range(numbers_fridge)]

for i in range(numbers_fridge):
    if virus(id_fridge[i]):
        print(i + 1, end=' ')
